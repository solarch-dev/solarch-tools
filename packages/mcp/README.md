...
| `get_unimplemented` | read-only | **Cerrahi AI'ın iş kuyruğu:** codegen'in bıraktığı `@solarch:surgical` işaretlerinden hâlâ `NOT_IMPLEMENTED` olan bölgeleri döner — iş açıklaması, fırlatılacak Exception'lar, kullanılabilir bağımlılıklar ve dosya/satır ile. Akış: get_unimplemented → bölgeyi doldur → check_drift. Tamamen lokal, login gerektirmez. |
